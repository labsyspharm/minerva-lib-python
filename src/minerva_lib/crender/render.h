 ...
 */
void clip16(uint16_t* target, uint16_t min, uint16_t max, int len);

/**
 * Clips values of target between min and max, and converts values to 0-255
 */
void clip32_conv8(uint32_t* target, uint8_t* output, uint16_t min, uint16_t max, int len);
 ...
 */
void rescale_intensity16(uint16_t* target, uint16_t min, uint16_t max, int len);
